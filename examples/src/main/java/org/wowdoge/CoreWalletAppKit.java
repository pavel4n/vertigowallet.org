 ...
 */
public class CoreWalletAppKit extends AbstractIdleService {
    ...
    private boolean storeFileLocked = false;
    private String spvFilePath;
    ...
    public CoreWalletAppKit setPeerNodes(PeerAddress... addresses) {
    ...
    public CoreWalletAppKit connectToLocalHost() {
        try {
    ...
    public CoreWalletAppKit setAutoSave(boolean value) {
        checkState(state() == State.NEW, "Cannot call after startup");
        useAutoSave = value;
        return this;
    }
     ...
     */
    public CoreWalletAppKit setDownloadListener(PeerEventListener listener) {
        this.downloadListener = listener;
        return this;
    }

    /** If true, will register a shutdown hook to stop the library. Defaults to true. */
    public CoreWalletAppKit setAutoStop(boolean autoStop) {
     ...
     */
    public CoreWalletAppKit setCheckpoints(InputStream checkpoints) {
     ...
     */
    public CoreWalletAppKit setBlockingStartup(boolean blockingStartup) {
     ...
     */
    public CoreWalletAppKit setUserAgent(String userAgent, String version) {
    ...
    public boolean isStoreFileLocked() {
    	return this.storeFileLocked;
    }
    ...
    protected void startUp() throws Exception {
        ...
        try {
            File chainFile = new File(spvFilePath); //new File(System.getProperty("user.home")), "dogecoins" + ".dogespvchain"); //(directory, filePrefix + ".dogespvchain"); //(new File("."), "wowdoge" + ".dogespvchain"); //
            boolean chainFileExists = chainFile.exists();
            vWalletFile = new File(directory, filePrefix); //filePrefix + ".dogewallet
            boolean shouldReplayWallet = vWalletFile.exists() && !chainFileExists;

            try {
            	vStore = new SPVBlockStore(params, chainFile);
            } catch (BlockStoreException e) {
            	this.storeFileLocked = true;
            	throw e;
            }
            if (checkpoints != null) {
				if (!chainFileExists && checkpoints != null) {
					// Ugly hack! We have to create the wallet once here to
					// learn the earliest key time, and then throw it
					// away. The reason is that wallet extensions might need
					// access to peergroups/chains/etc so we have to
					// create the wallet later, but we need to know the time
					// early here before we create the BlockChain
					// object.
            ...
            if (peerAddresses != null) {
                ...
                vPeerGroup.addPeerDiscovery(new DnsDiscovery(params)); //new IrcDiscovery(channel));//new DnsDiscovery(params));
            }
    ...
    private void installShutdownHook() {
        if (autoStop) Runtime.getRuntime().addShutdownHook(new Thread() {
            @Override public void run() {
                try {
                    CoreWalletAppKit.this.stopAndWait();
                } catch (Exception e) {
                    throw new RuntimeException(e);
                }
            }
